...
- Aplica imagem do próprio artigo (download/cache em assets/cache)
...
Env:
  NEWSDATA_API_KEY (ou NEWS_API_KEY)
"""
...
USER_AGENT = "ai-discovery-bot/1.3 (+github actions)"
...
def fetch_articles(limit: int = MAX_POSTS) -> List[Dict[str, Any]]:
    if not API_KEY:
        raise ValueError("API KEY not set (NEWSDATA_API_KEY ou NEWS_API_KEY).")
...
def main():
    try:
        ensure_dir(ASSET_CACHE_DIR)
        # cria .gitkeep para garantir que a pasta vai ao repo mesmo vazia
